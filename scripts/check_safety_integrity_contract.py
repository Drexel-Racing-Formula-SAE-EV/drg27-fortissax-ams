...
def main() -> int:
    ...
    require("# CONFIG_AMS_BMS_AUTHORITY is not set" in config,
            "BMS_OK assertion authority enabled during Z-012")
    require("# CONFIG_AMS_BALANCE_AUTHORITY is not set" in config,
            "balancing authority enabled during Z-012")
    ...
    # Only the migrated fan workload may currently count as real safety
    # heartbeat evidence. Current/ADBMS/CAN/estimator remain placeholders.
    ...
    require(".safety_evidence_ready = true" not in
            (runtime[:fan_start] + runtime[air_start:]),
    ...
    print("PASS: Z-012 deep fail-low/runtime-integrity safety contract")
    return 0
