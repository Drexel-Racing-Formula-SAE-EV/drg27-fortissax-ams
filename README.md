...
Two narrowly approved hardware-specific seams exist because the generic stack
cannot provide the required safety behavior: the board-owned PE0 BMS_OK
emergency fail-low primitive, and the Z-015 private STM32F767 SPI6 polling
backend. The latter is confined to `drivers/ams/adbms_spi_stm32.c`; its raw
transfer API is private and has zero runtime callers at Z-015.
...
bounded SPI engine, optimization/model campaign, sanitizers, static analyzers,
and mutation controls. It intentionally does not target-build/flash, perform
physical SPI/IWDG validation, or advance to Z-016.

The canonical host/SIL closeout completed successfully with 53/53 stages, ThreadSanitizer requested/performed, and zero skipped evidence.
